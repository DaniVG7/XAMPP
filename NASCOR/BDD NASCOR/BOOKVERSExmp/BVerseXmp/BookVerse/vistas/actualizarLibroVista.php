 ...
 <h2>Modificar datos del título <?php echo $datosLibro['titulo'];?></h2>
  <div class="formcurso">
	<form method="POST" action="../controladores/actualizar2LibroControlador.php">
		...
		<label>Número de páginas:</label><input type="number"  name="numPaginas" value="<?php echo $datosLibro['numPaginas'] ?>" ><br>
		...
		foreach($listaAutores->autores as $autor){
			...
			if ($autor['idAutor'] === $datosLibro['idAutor']){
				echo 'selected';
			}
			echo ">".$autor['nombre'].' '.$autor['apellido1'].' '.$autor['apellido2'].'</option>';
		}
	    ?>
		</select>
