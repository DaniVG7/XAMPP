...
<form action = "modificar2CursoControlador.php" method="post">	
	...
	<label><strong>Personal docente:  </strong></label><select name="idProfesor" required>
		...
		foreach ($profesores->profesores as $profesor) {
			echo '<option value="' . $profesor['idProfesor'] . '"';
			if ($profesor['idProfesor'] == $curso['idProfesor']) {
				echo ' selected';
			}
			echo '>' . $profesor['nombreProfesor'] . ' ' . $profesor['apellido1Profesor'] . ' ' . $profesor['apellido2Profesor'] . '</option>';
		}
		?>   
</select><br><br>
	...
	<option selected disabled value="">Seleccione un aula</option>
    	<?php 
            ...
            echo '<option value="' . $aula['idAula'] . '"';
				if($aula['idAula'] == $curso['idAula']){
				 echo 'selected';
				}
					
			echo'>' . $aula['nombre'].'</option>';
        }
    ?>
	</select><br>
