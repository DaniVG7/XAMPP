...
//echo '<p>Gracias sr. '.$_POST['nombre'].' '.$_POST['apellido1'].' '.$_POST['apellido2'].' por enviar su candidatura, hemos recibido sus datos correctamente.</p>';
...
try {
    ...
    //Receptores
    $mail->setFrom('dvg9383@gmail.com', 'De: RRHH Developery'); //Correo desde donde se envia y el nombre. En el correo saldrá el mail en la bandeja de enviados
    $mail->addAddress($persona['Mail'],'Copia para: '.$persona['Nombre'].' De :dvg9383@gmail.com');// Correo receptor. Este será para el usuario que rellena el formulario

	//Esta linea es para añadir una copia en este caso sería el correo de la empresa (no puede ser el mismo que el correo que envia todo (dvg9383@gmail.com) ya que GMAIL no deja enviarse un correo a sí mismo.)
	$mail->addCC('carlosjordonezb@gmail.com', 'Copia para la empresa de: '.$persona['Nombre'].' '.$persona['Apellido1'].' '.$persona['Apellido2']); // Genera una copia
	...
	echo '<span style= "color:green"><strong>Candidatura enviada correctamente✅</strong></span>';//Mensaje de éxito
	echo '<p>Estimado sr. '.$_POST['nombre'].' '.$_POST['apellido1'].' '.$_POST['apellido2'].': <br>';
	echo 'Desde Developery hemos recibido sus datos por correo para la oferta de '.$_POST['puestoTrabajo'].'.<br>
	Gracias por el interés, en breve recibirá un correo en '.$_POST['mail'].' donde podrá comprobar los datos que hemos recibido.<br>
	Nos pondremos en contacto con usted próximamente para informarle sobre el estado de su candidatura.';
} catch (Exception $e) { //Si algo falló y no se pudo enviar mostrará el siguiente mensaje:
    echo "❌Error al intentar enviar el correo. Desafortunadamente no pudimos recibir sus datos. <br>
	Trate de rellenar de nuevo el formulario y volverlo a enviar. Recuerde que el correo de contacto debe existir.<br>
	Motivo del error: <span style= 'color:red'>{$mail->ErrorInfo}</span>⚠️⚠️";
}
