class Persona {
  ...
  function añadirPersona() {
    ...
    listaPersonas = personas.map((per)=>per.apellido1 + ' ' + per.apellido2 + ', ' + per.nombre);//Esto es lo mismo que la funcion de abajo.
    //function nombrePersona(per){
    //    return [per.apellido1 + ' ' + per.apellido2 + ', ' + per.nombre];
    //}
    //Ahora podemos alfabeticamente ordenar los elementos con el metodo sort().
    listaPersonas.sort();
    //guardar en el navegador el array de personas
    let personasJSON = JSON.stringify(personas);
    localStorage.setItem("personas", personasJSON);
    //Mostramos el array Personas con los datos almacenados alfabéticamente y separados con un br por cada elemento
    
    document.getElementById('resultado').innerHTML = listaPersonas.join('<br>')
    }
}
