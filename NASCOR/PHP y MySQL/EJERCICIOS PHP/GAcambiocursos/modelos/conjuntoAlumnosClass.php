...
/*	public function leerPersonas() {
	...
	public function buscarAlumnosPorCurso($id){
		foreach ($this->alumnos as $alu) {
			if ($alu->getCurso()==$id) {
				$alumnosCurso[]=$alu;
			}
		}
	...
	public function cambiarCursos($cursoAntiguo,$cursoNuevo) {
		foreach ($this->alumnos as $alu) {
			if ($alu->getCurso()==$cursoAntiguo) {
				$alu->setCurso($cursoNuevo);
			}
		}		
	}
