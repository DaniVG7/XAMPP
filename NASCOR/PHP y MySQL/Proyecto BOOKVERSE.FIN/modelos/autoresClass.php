...
class conjuntoAutores extends connect{
	...
	public function consultarLibrosAutor($idAutor){
		if (isset($_SESSION['username'])) {
   		 	$sql = "SELECT GB_Persona.nombre AS nombreAutor, GB_Persona.apellido1 AS ape1Autor, GB_Persona.apellido2 AS ape2Autor, 						GB_Libro.titulo, GB_Libro.generoLiterario, GB_Libro.añoPublicacion, GB_Libro.numPaginas, GB_Libro.idioma, 								GB_Libro.premios, GB_Libro.sinopsisLibro, GB_Libro.imagen 
	...
	/*public function borrarAutor($idAutor) {
		$sql= "SELECT FROM GB_Autor_Libro WHERE GB_Autor_Libro = $idAutor";
		$query = $this->db->query($sql);
		// Consulta SQL para eliminar un autor por su ID
        $sql="DELETE FROM GB_Autor_Libro WHERE `GB_Autor_Libro`.`idAutor` = $idAutor"; //primero borramos la relacion autor-libro.
		$query = $this->db->query($sql);
		$sql="DELETE FROM GB_Autor WHERE GB_Autor.idAutor = $idAutor"; // luego borramos el gb autor
		$query = $this->db->query($sql);
		$sql="DELETE FROM GB_Persona WHERE GA_Persona.idPersona = $idAutor"; // por último borramos la persona que tenia el mismo id autor.
		$query = $this->db->query($sql);		
	}*/
	public function borrarAutor($idAutor) {
		if (isset($_SESSION['username'])) {
